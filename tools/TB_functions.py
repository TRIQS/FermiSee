...
def get_kx_ky_FS(X, Y, origin, tbl, k_trans_back, N_kxy=10, fermi=0.0):
    kx = np.linspace(0,0.5,N_kxy)
    ky = np.linspace(0,0.5,N_kxy)

    # this is for projections in future
    # if select is None:
    #     select = np.array(range(tbl.n_orbitals))

    E_FS = np.zeros((tbl.n_orbitals,N_kxy,N_kxy))
    for kyi in range(N_kxy):
        path_FS = [((Y-origin)/(N_kxy-1)*kyi + origin,
                    origin+(X-origin)+(Y-origin)/(N_kxy-1)*kyi)]
